...
export default function NotesScreen() {
  ...
  const openNoteModal = (note: Note) => {
    ...
    if (note.blocks && note.blocks.length > 0) {
      ...
      console.log('[Notes] Cargando blocks:', blocksWithIds.map(b => ({ id: b.id, type: b.type })));
  ...
  const saveNoteAuto = async () => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        console.log('[Notes] Guardando nota:', {
          id: selectedNote.id,
  ...
  const deleteChecklistBlock = async (blockId: string) => {
    console.log('[Notes]? Eliminando checklist block:', blockId);
    console.log('[Notes] Blocks antes:', editingBlocks.map(b => ({ id: b.id, type: b.type })));
    ...
    setEditingBlocks(prev => {
      ...
      console.log('[Notes] Blocks después:', filtered.map(b => ({ id: b.id, type: b.type })));
      return filtered;
    });
