...
export default function DatePickerSetting({
  ...
  useEffect(() => {
    ...
    if (selectedEvent) {
      // Evento existente - soportar MonthEvent sin date/startTime
      if ('startDay' in selectedEvent && 'year' in selectedEvent && 'month' in selectedEvent) {
        const year = selectedEvent.year;
        const month = selectedEvent.month;
        const day = selectedEvent.startDay;
        const eventDate = new Date(Date.UTC(year, month, day, 0, 0, 0, 0));
        initialDate = eventDate;
        initialTime = eventDate;
      } else {
        // Evento normal - usar su fecha y hora
